...
function LocalMultiplayerSetup({player, setGameSettings}) {
    ...
    // Initialize with previous players if available, otherwise default setup
    const [localPlayers, setLocalPlayers] = useState(() => {
        if (previousPlayers.length > 0) {
            // Use previous players but ensure the host is the current player
            const updatedPlayers = [...previousPlayers]
            const hostIndex = updatedPlayers.findIndex((p) => p.isHost)

            if (hostIndex !== -1) {
                // Update the host with current player info
    ...
    useEffect(() => {
        console.log("LocalMultiplayerSetup - localPlayers:", localPlayers)
    }, [localPlayers])
    ...
    const handleRemovePlayer = (index) => {
        ...
        // Ensure player IDs are sequential
        updatedPlayers.forEach((p, i) => {
            if (i > 0) {
                // Skip the host player
                p.id = `player${i + 1}`
            }
        })
    ...
    const handleStartGame = () => {
        ...
        console.log("Starting game with players:", localPlayers)
    ...
    return (
        <div className="game-container py-8 md:py-12">
            <div className="max-w-4xl mx-auto">
                ...
                {error && (
                    <div
                        className="mb-6 p-3 bg-red-500/20 border border-red-500/30 rounded-lg text-red-200 text-sm animate-slide-in">
                        {error}
                    </div>
                )}
